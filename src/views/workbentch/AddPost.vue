        ...
        <el-form label-width="120px" :model="CoverForm">
          ...
          <el-form-item label="项目">
            <el-input
              size="small"
              placeholder="请输入楼盘项目"
              v-model="CoverForm.project"
              @blur="onProjectLoseFocus"
            ></el-input>
          </el-form-item>
          <el-form-item label="楼盘区域">
            <el-input
              size="small"
              placeholder="请输入所属楼盘区域"
              v-model="CoverForm.projectDistrict"
              @blur="saveCoverOnLoseFocus"
            ></el-input>
          </el-form-item>
            ...
            <el-col :span="11">
              <el-form-item>
                <el-input
                  size="small"
                  placeholder="纬度"
                  v-model.number="CoverForm.latitude"
                  type="number"
                  @blur="saveCoverOnLoseFocus"
                ></el-input>
              </el-form-item>
            ...
            <el-col :span="12" :offset="1">
              <el-form-item>
                <el-input
                  size="small"
                  placeholder="经度"
                  v-model.number="CoverForm.longitude"
                  type="number"
                  @blur="saveCoverOnLoseFocus"
                ></el-input>
              </el-form-item>
          ...
          <el-form-item label="选择价格">
            ...
            <el-select
              size="small"
              class="width-full"
              placeholder="请选择楼盘关联价格"
              v-model="CoverForm.buildingId"
              @change="setBuildingId(CoverForm.buildingId)"
              @blur="saveCoverOnLoseFocus"
            >
              <el-option
                v-for="p in priceAvail"
                :key="p.buildingBaseId"
                :label="p.project"
                :value="p.buildingBaseId"
              >
          ...
          <el-form-item label="选择标签">
            <div class="inline" v-for="(tag,key) in CoverForm.tags" :key="key">
              <el-tag
                :type="tag.type=='ADVANTAGE'?'success':'danger'"
                closable
                @close="onTagRemove(key)"
              >{{tag.name}}</el-tag>
            </div>
            <el-row>
              <el-col :span="5">
      ...
      <el-col :span="20">
        <el-tabs v-model="activeDevisionName" :tab-position="'left'">
          <el-tab-pane
            :label="devision.name"
            :name="devision.name"
            v-for="(devision,devisionKey) in CoverForm.devisions"
            :key="devisionKey"
          >
            <el-form label-width="80px">
              <el-row class="devision">
                <el-form-item label="分数" v-model="devision.sort">
                  ...
                  <el-col :span="8">
                    <el-input
                      size="medium"
                      v-model="devision.mark"
                      type="number"
                      placeholder="请输入分数"
                      @blur="saveDevisionOnLoseFocus(devisionKey)"
                    ></el-input>
                  </el-col>
                </el-form-item>
                  ...
                  >
                    ...
                    <el-table-column prop="must" label="必填" width="120">
                      <template slot-scope="scope">
                        <span v-if="scope.row.must==true">
                          <i class="el-icon-success" style="color:red;"></i>
                        </span>
                        <span v-if="scope.row.must==false">
                          <i class="el-icon-error" style="color:green;"></i>
                        </span>
                      </template>
                    </el-table-column>
                    ...
                    <el-table-column label="内容">
                      <template slot-scope="scope">
                        <el-input
                          ...
                          @blur="saveParamsOnLoseFocus(devisionKey)"
                        ></el-input>
                      </template>
                    </el-table-column>
                  </el-table>
              ...
              <el-row
                v-for="(_module,moduleKey) in devision.moduleList"
                :key="moduleKey"
                class="sub-devision"
              >
                <div class="margin-up-down-20">
                  <el-alert
                    title="子栏目"
                    type="success"
                    :center="true"
                    :closable="true"
                    close-text="删除"
                    @close="deleteModule(devisionKey,moduleKey)"
                  ></el-alert>
                </div>
                <el-form-item label="标题">
                  <el-col :span="24">
                    <el-input
                      ...
                      placeholder="请输入子栏目的标题"
                      @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                    ></el-input>
                  </el-col>
                </el-form-item>
                <!-- <el-form-item label="描述">
                ...
                <el-form-item label="属性">
                  <el-col :span="3">
                    <el-select
                      size="small"
                      placeholder="选择分支"
                      v-model="_module.branch"
                      @change="onBranchChange(devisionKey,moduleKey,$event)"
                      @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                    >
                  ...
                  <el-row>
                    ...
                    <el-col :span="24" v-if="_module.branch=='TITLE_IMGS_CONTENT'">
                      <div v-for="(list_item,list_key) in _module.content" :key="list_key">
                        ...
                        <e-uploader
                          v-model="list_item.img"
                          @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                        ></e-uploader>
                        <div class="margin-up-down-10">
                          <el-input
                            size="small"
                            placeholder="输入图片的名字，使用“|”分割"
                            v-model="list_item.imgNames"
                            @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                          ></el-input>
                        </div>
                        <div class="margin-up-down-10 h-right">
                          <el-button
                            ...
                            size="small"
                            type="warning"
                            v-if="_module.content.length>1"
                            @click="onImgContentListRemove(devisionKey,moduleKey,list_key)"
                          >删除此节</el-button>
                        </div>
                      </div>
                      <div class="h-right">
                        <el-button
                          size="small"
                          type="primary"
                          @click="onImgContentListAdd(devisionKey,moduleKey)"
                        >继续添加</el-button>
                      </div>
                    </el-col>
                    <el-col :span="24" v-if="_module.branch=='TITLE_LIST'">
                      <div
                        v-for="(property,pkey) in _module.content"
                        :key="pkey"
                        class="margin-up-down-10"
                      >
                        <el-alert
                          :type="property.type=='ADVANTAGE'?'success':'error'"
                          :title="property.content"
                          @close="onContentListRemove(devisionKey,moduleKey,pkey)"
                        ></el-alert>
                      </div>
                      <div class="margin-up-down-10">
                        <el-input
                          size="small"
                          v-model="temModuleContent.content"
                          @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                        >
                          <el-select
                            size="small"
                            class="width-120"
                            v-model="temModuleContent.type"
                            slot="prepend"
                            placeholder="请选择类型"
                            @blur="saveModuleOnLoseFocus(devisionKey,moduleKey)"
                          >
                          ...
                          <el-button
                            ...
                            slot="append"
                            type="success"
                            @click="onContentListAdd(devisionKey,moduleKey)"
                          >添加</el-button>
                        </el-input>
                      </div>
                    </el-col>
